...
- **Timetable/Course Separation:** Timetable imports save timetable slots only. The CWA/GPA current-semester import options include `Use Timetable`, which deduplicates timetable courses, excludes courses already in the active semester, and requires the student to enter credit hours before creating `CourseModel` records.
